DROP VIEW IF EXISTS weekft;

CREATE VIEW weekft AS
WITH daily_features AS (
	SELECT 
	date,
	-- Add temporal features (month, day of week, day number, weekend and holiday marker)
...
SELECT
	...
	is_prov_elec, is_nat_elec, week_of_year,
    
      -- Counts holidays
    SUM(is_holiday) AS total_holidays,
    
    -- Cyclical features
    ROUND(SIN(2 * PI() * week_of_year / 52)::NUMERIC,2) AS week_of_year_sin,
    ROUND(COS(2 * PI() * week_of_year / 52)::NUMERIC,2) AS week_of_year_cos


FROM daily_features AS d
GROUP BY week_start, week_of_year, is_prov_elec, is_nat_elec
ORDER BY week_start;
