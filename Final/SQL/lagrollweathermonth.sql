...
WITH monthly_weather AS (
	...
	SUM (is_freeze_thaw) AS freezethaw_days,
	ROUND(AVG(ABS(days_since_snow))::NUMERIC) AS avgdayssincesnow,
	MAX(consecutive_snow) AS max_consecutive_snow
...
-- Obtain lag features for weather data (1,4 and 12 months)
	...
	-- Lag Snow Days
	LAG(snow_days,1) OVER (ORDER BY month_start) AS lagsnowdays1,
	LAG(avgdayssincesnow,1) OVER (ORDER BY month_start) AS lagavgdaysincesnow1,
	LAG(freezethaw_days,1) OVER (ORDER BY month_start) AS lagfreezethaw_days1,
	...
	--Rolling Window Sum (Climate Indicators) over 2 and 4 months

	-- Freeze Thaw Days
	ROUND(SUM(freezethaw_days) OVER (ORDER BY month_start
		ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING)::NUMERIC,2) AS sumfreezethaw2,
	-- Snow Days
	ROUND(SUM(snow_days) OVER (ORDER BY month_start
			ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING)::NUMERIC,2) AS sumsnowdays2,
	-- Average Days since Snow
	ROUND(SUM(avgdayssincesnow) OVER (ORDER BY month_start
			ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING)::NUMERIC,2) AS sumavgdayssincesnow2,
	-- Precipitation (Rain)
	...
	-- Freeze Thaw Days
	ROUND(STDDEV(freezethaw_days) OVER (ORDER BY month_start
		ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING)::NUMERIC,2) AS stdfreezethaw,
	-- Snow Days
	ROUND(STDDEV(snow_days) OVER (ORDER BY month_start
			ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING)::NUMERIC,2) AS stdsnowdays2,
	-- Average Days since Snow
	ROUND(STDDEV(avgdayssincesnow) OVER (ORDER BY month_start
			ROWS BETWEEN 2 PRECEDING AND 1 PRECEDING)::NUMERIC,2) AS stdavgdayssincesnow2,
